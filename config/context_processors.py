...
def admin_new_agent_requests(request):
    """
    For /admin/ pages: count of new agent requests (yellow banner in base_site).
    Skips DB work for non-staff and non-admin URLs.
    """
    path = getattr(request, 'path', '') or ''
    if not path.startswith('/admin/'):
    ...
    if not user or not user.is_authenticated or not user.is_staff:
        return {}
    from apps.accounts.models import AgentRequest

    n = AgentRequest.objects.filter(status=AgentRequest.Status.NEW).count()
    return {'admin_new_agent_requests_count': n}
